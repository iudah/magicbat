...
static inline void fused_attention_gt_block_size_fast(
    ...
    const f32 *restrict v_data, f32 *restrict r_data, f32 sqrt_d, f32 *max,
    f32 *sum) {
...
fused_attention_gt_block_size(u32 b_len, u32 m_len, u32 n_len, u32 k_len,
                              ...
                              const f32 *v_data, f32 *r_data, f32 sqrt_d,
  ...
  MATMUL_OUTER_LOOP(b) {
    MATMUL_OUTER_LOOP(m) {
      MATMUL_OUTER_LOOP(n) {
        ...
        fused_attention_gt_block_size_fast(b_outer, m_outer, n_outer, m_len,
                                           n_len, k_len, q_data, k_data, v_data,
                                           r_data, sqrt_d, max_data, sum_data);
      }
      MATMUL_INNER_LOOP(b) {
        MATMUL_INNER_LOOP(m) {
...
static inline void fused_attention_gt_block_size_unbatched_fast(
    ...
    f32 *restrict r_data, f32 sqrt_d, f32 *max, f32 *sum) {
...
static inline f32 *fused_attention_lt_block_size(
    u32 m_len, u32 n_len, u32 k_len, const f32 *q_data, const f32 *k_data,
    const f32 *v_data, f32 *r_data, f32 sqrt_d, f32 *max_data, f32 *sum_data) {
