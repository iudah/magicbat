#ifndef FUSED_ATTENTION_BACKWARD_H
#define FUSED_ATTENTION_BACKWARD_H

#include "matmul.h"
#include "matmul_tile.h"
#include <math.h>

static inline void fused_attention_backward_gt_block_size_fast(
    ...
    f32 *restrict dv_data, const f32 *restrict dr_data, f32 sqrt_d,
...
static inline void fused_attention_backward_gt_block_size(
  ...
  MATMUL_OUTER_LOOP(b) {
    MATMUL_OUTER_LOOP(m) {
      MATMUL_OUTER_LOOP(n) {
        ...
        fused_attention_backward_gt_block_size_fast(
            b_outer, m_outer, n_outer, m_len, n_len, k_len, q_data, k_data,
            v_data, dq_data, dk_data, dv_data, dr_data, sqrt_d, max_data,
            sum_data, row_sum);
      }
    }
  }
...
static inline void fused_attention_backward_gt_block_size_unbatched_fast(
...
static inline void fused_attention_backward_gt_block_size_unbatched(
  ...
  MATMUL_OUTER_LOOP(m) {
    MATMUL_OUTER_LOOP(n) {
      ...
      fused_attention_backward_gt_block_size_unbatched_fast(
          m_outer, n_outer, k_len, q_data, k_data, v_data, dq_data, dk_data,
          dv_data, dr_data, sqrt_d, max_data, sum_data, row_sum);
    }
  }
}

static inline void fused_attention_backward_lt_block_size(
    u32 m_len, u32 n_len, u32 k_len, const f32 *q_data,
    const f32 *restrict k_data, const f32 *restrict v_data,
    f32 *restrict dq_data, f32 *restrict dk_data, f32 *restrict dv_data,
    const f32 *restrict dr_data, f32 sqrt_d, const f32 *restrict max,
