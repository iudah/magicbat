...
static inline void bmm_non_contiguous_no_bc(Tensor tensor_a, Tensor tensor_b,
                                            ...
                                            bool UNUSED_ARG broadcast_a,
                                            bool UNUSED_ARG broadcast_b);
Tensor tensor_bmatmul_transpose_a(const Tensor tensor_a,
                                  const Tensor tensor_b) {
  if (bmm_tensors_invalid(tensor_a, tensor_b, true, false))
    return nullptr;
  ...
  u32 row = tensor_a->shape[tensor_a->ndims - 1];
    ...
    for (u32 j = 0; j < com; ++j) {
      for (u32 i = 0; i < row; ++i) {
        ...
        for (u32 k = 0; k < col; ++k) {
          ...
          f32 b_val = tensor_get(tensor_b, (u32[MAX_DIMS]){batch_index, j, k});
    ...
    for (u32 j = 0; j < com; ++j) {
      for (u32 i = 0; i < row; ++i) {
        ...
        for (u32 k = 0; k < col; ++k) {
          ...
          f32 a_val = tensor_get(
              tensor_a, (u32[MAX_DIMS]){broadcast_a ? 0 : batch_index, j, i});
          f32 b_val = tensor_get(
    ...
    for (u32 j = 0; j < com; ++j) {
      for (u32 i = 0; i < row; ++i) {
        ...
        for (u32 k = 0; k < col; ++k) {
          ...
          auto idx_a_0 = broadcast_a ? j : batch_index;
          auto idx_a_1 = broadcast_a ? i : j;
    ...
    for (u32 j = 0; j < com; ++j) {
      for (u32 i = 0; i < row; ++i) {
        ...
        for (u32 k = 0; k < col; ++k) {
          ...
          auto a_val =
              tensor_get(tensor_a, (u32[MAX_DIMS]){idx_a_0, idx_a_1, i});
          auto b_val =
              tensor_get(tensor_b, (u32[MAX_DIMS]){idx_b_0, idx_b_1, k});
