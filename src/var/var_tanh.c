...
void tanh_backward_fn(Var self) {
  ...
  if (a && !a->base.is_tensor_type && a->base.requires_grad) {
    ...
    auto tmp = tensor_tanh_backward((Tensor)a, self->grad);
...
Tensor var_tanh(Tensor a) {

  Tensor tmp = tensor_tanh(a);
  if (!tmp)
    return nullptr;
  ...
  var_track_parent(res, a, nullptr, (VarOp){tanh_backward_fn, nullptr}, nullptr);
