...
int main(void) {
  ...
  /* ---------------- Softmax along axis 1 (rows) ---------------- */
  {
    ...
    Tensor softmaxed =
        tensor_softmax(t, 1); // softmax along last axis (columns)
    assert(softmaxed != nullptr);

    // Check that each row sums to \~1.0
    // Row 0
    float row0_sum = softmaxed->data->data[0] + softmaxed->data->data[1] +
                     softmaxed->data->data[2];
    assert(fabsf(row0_sum - 1.0f) < EPS);

    // Row 1 (all zeros → uniform distribution)
    float row1_sum = softmaxed->data->data[3] + softmaxed->data->data[4] +
                     softmaxed->data->data[5];
    assert(fabsf(row1_sum - 1.0f) < EPS);

    // Check that row 0 has increasing probabilities (because 1 < 2 < 3)
    assert(softmaxed->data->data[0] < softmaxed->data->data[1]);
    assert(softmaxed->data->data[1] < softmaxed->data->data[2]);
  ...
  /* ---------------- Softmax along axis 0 (columns) ---------------- */
  {
    ...
    // Column 0: 1, 2, 3
    // Column 1: 0, 0, 0
    t->data->data[0] = 1.0f; // row0 col0
    t->data->data[1] = 0.0f; // row0 col1
    t->data->data[2] = 2.0f; // row1 col0
    t->data->data[3] = 0.0f;
    t->data->data[4] = 3.0f; // row2 col0
    t->data->data[5] = 0.0f;

    Tensor softmaxed = tensor_softmax(t, 0); // softmax down each column
    assert(softmaxed != nullptr);

    // Each column should sum to \~1.0
    float col0_sum = softmaxed->data->data[0] + softmaxed->data->data[2] +
                     softmaxed->data->data[4];
    float col1_sum = softmaxed->data->data[1] + softmaxed->data->data[3] +
                     softmaxed->data->data[5];
